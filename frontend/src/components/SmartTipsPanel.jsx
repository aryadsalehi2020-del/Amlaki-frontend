...
function SmartTipsPanel({ profile, immobilie }) {
  ...
  const relevanteTipps = useMemo(() => {
    ...
    if (immobilie?.verwendungszweck === 'kapitalanlage') {
      tipps.push({ kategorie: 'Steuer-Optimierung', titel: 'Kaufpreisaufteilung optimieren', prioritaet: 'mittel', ersparnis: kaufpreis * 0.015, ersparnisText: `${formatCurrency(kaufpreis * 0.015)} uber 50 Jahre moglich`, zusammenfassung: 'Gebaudeanteil maximieren fur hohere AfA', details: ['Nur Gebaudeanteil ist abschreibbar', 'Eigenes Gutachten lohnt sich!', 'Gutachterkosten: 1.500-3.000 EUR'], aktion: 'Gutachter suchen' });
    }
    ...
    <div className="space-y-6">
      ...
      {relevanteTipps.length === 0 ? (
        <div className="bg-white p-8 rounded-[16px] border border-[#E8E0D4] text-center">
          <h4 className="text-[16px] font-semibold text-[#2C2418] mb-2">Profil ausfullen fur Tipps</h4>
          <p className="text-[#8C7E6A] text-[14px]">Fullen Sie Ihr Finanzprofil aus, um personalisierte Spar-Tipps zu erhalten</p>
        </div>
      ) : (
                ...
                <button onClick={() => toggleTipp(relevanteTipps.indexOf(tipp))} className="w-full p-5 flex items-start gap-4 text-left hover:bg-[#FAF7F2] transition-colors">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1 flex-wrap">
                      ...
                      {tipp.prioritaet === 'hoch' && <span className="px-2 py-0.5 bg-[#7C8B6F]/10 text-[#7C8B6F] text-[11px] rounded-full font-semibold">Hohe Prioritat</span>}
                    </div>
...
function getLandesfoerderung(bundesland) {
  const foerderungen = {
    'NRW': { kategorie: 'Landesforderung', titel: 'NRW.BANK Eigentumsforderung', prioritaet: 'hoch', ersparnis: 40000, ersparnisText: 'Zinssatz nur 0,5%!', zusammenfassung: 'Extrem gunstiges Forderdarlehen fur NRW-Burger', details: ['Forderdarlehen 100.000-184.000 EUR moglich', 'Extrem gunstiger 0,5% Zins', 'Einkommensgrenzen: ca. 62.000 EUR (Familie mit 2 Kindern)', 'Kombinierbar mit KfW-Forderung'], aktion: 'NRW.BANK Rechner nutzen' },
    'Brandenburg': { kategorie: 'Landesforderung', titel: 'ILB Wohneigentumsforderung', prioritaet: 'hoch', ersparnis: 80000, ersparnisText: 'Bis 230.000 EUR ZINSFREI!', zusammenfassung: 'Die beste Landesforderung Deutschlands', details: ['Darlehen bis 230.000 EUR komplett ZINSFREI!', 'Gilt fur Familien mit Kindern', 'Beste Immobilienforderung in ganz Deutschland'], aktion: 'ILB kontaktieren' },
    'Bayern': { kategorie: 'Landesforderung', titel: 'BayernLabo Eigenwohnraumforderung', prioritaet: 'mittel', ersparnis: 25000, ersparnisText: 'Bis 3% unter Marktzins', zusammenfassung: 'Gunstige Konditionen plus niedrigste GrESt', details: ['Zinsverbilligung bis zu 3 Prozentpunkte', 'Nur 3,5% Grunderwerbsteuer in Bayern!'], aktion: 'BayernLabo Rechner nutzen' },
  };
