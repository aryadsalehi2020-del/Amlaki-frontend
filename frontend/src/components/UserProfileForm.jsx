...
function UserProfileForm({ profile, onProfileChange }) {
  ...
  const bundeslaender = ['Baden-W\u00fcrttemberg', 'Bayern', 'Berlin', 'Brandenburg', 'Bremen', 'Hamburg', 'Hessen', 'Mecklenburg-Vorpommern', 'Niedersachsen', 'NRW', 'Rheinland-Pfalz', 'Saarland', 'Sachsen', 'Sachsen-Anhalt', 'Schleswig-Holstein', 'Th\u00fcringen'];
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-[16px] p-6 border border-[#E8E0D4]">
        ...
        <div className="grid md:grid-cols-2 gap-6">
          <div><label className={labelClass}>Jahreseinkommen brutto (EUR)</label><input type="number" name="jahreseinkommen" value={formData.jahreseinkommen} onChange={handleChange} placeholder="z.B. 60000" className={inputClass} /><p className="text-[12px] text-[#8C7E6A] mt-1">Wichtig f\u00fcr F\u00f6rderungsgrenzen</p></div>
          <div><label className={labelClass}>Verf\u00fcgbares Eigenkapital (EUR)</label><input type="number" name="eigenkapital" value={formData.eigenkapital} onChange={handleChange} placeholder="z.B. 50000" className={inputClass} /><p className="text-[12px] text-[#8C7E6A] mt-1">Bargeld + kurzfristig verf\u00fcgbar</p></div>
      ...
      <div className="bg-white rounded-[16px] p-6 border border-[#E8E0D4]">
        ...
        <div className="grid md:grid-cols-2 gap-6">
          <div><label className={labelClass}>Besch\u00e4ftigungsart</label><select name="beruf" value={formData.beruf} onChange={handleChange} className={inputClass}><option value="angestellt">Angestellt (unbefristet)</option><option value="beamte">Beamte/r</option><option value="selbststaendig">Selbstst\u00e4ndig/Freiberufler</option><option value="befristet">Befristet angestellt</option><option value="rentner">Rentner/in</option></select></div>
          <div><label className={labelClass}>Bundesland</label><select name="bundesland" value={formData.bundesland} onChange={handleChange} className={inputClass}>{bundeslaender.map(bl => (<option key={bl} value={bl}>{bl}</option>))}</select><p className="text-[12px] text-[#8C7E6A] mt-1">F\u00fcr Landesf\u00f6rderung + Grunderwerbsteuer</p></div>
        </div>
      ...
      <div className="bg-white rounded-[16px] p-6 border border-[#E8E0D4]">
        ...
        <div className="grid md:grid-cols-2 gap-6">
          <div><label className={labelClass}>Anzahl Kinder</label><select name="kinder" value={formData.kinder} onChange={handleChange} className={inputClass}><option value={0}>Keine Kinder</option><option value={1}>1 Kind</option><option value={2}>2 Kinder</option><option value={3}>3 Kinder</option><option value={4}>4+ Kinder</option></select><p className="text-[12px] text-[#7C8B6F] mt-1">Wichtig! Kinder erh\u00f6hen KfW-F\u00f6rderung erheblich</p></div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-[16px] p-6 border border-[#E8E0D4]">
        ...
        <div className="space-y-4">
          {[
            { name: 'hatDepot', label: 'Wertpapierdepot vorhanden', valueName: 'depotWert', hint: 'Lombardkredit m\u00f6glich: bis 70% beleihbar', placeholder: 'Depotwert in EUR' },
            { name: 'hatLebensversicherung', label: 'Kapital-Lebensversicherung', valueName: 'lvRueckkaufswert', hint: 'Policendarlehen m\u00f6glich: bis 100% beleihbar', placeholder: 'R\u00fcckkaufswert in EUR' },
            { name: 'hatRiester', label: 'Riester-Vertrag', valueName: 'riesterGuthaben', hint: 'Wohn-Riester: 100% f\u00fcr Immobilienkauf entnehmbar', placeholder: 'Guthaben in EUR' },
            { name: 'hatBausparvertrag', label: 'Bausparvertrag', valueName: 'bausparGuthaben', hint: 'Direktes Eigenkapital + g\u00fcnstiges Bauspardarlehen', placeholder: 'Guthaben in EUR' },
          ].map(item => (
